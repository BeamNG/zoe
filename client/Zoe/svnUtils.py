...
def svnCheckout(targetRevision: str="HEAD", **execCtx):
  ...
  ret, _ = exec('svn checkout {vcs.url:} . --config-option config:miscellany:use-commit-times=yes --non-interactive --username "{vcs.username:}" --password "{vcs.password:}" -r {targetRevision}''', callbackClass=reporter, **execCtx)
  return ret == 0
...
def svnResetPath(**execCtx):
  try:
    ...
    logger.error("Unable to reset svn path {} - {}".format(execCtx['vcs'].outPath, e))
    return False
  return True
...
def svnUpdate(targetRevision="HEAD", **execCtx):
  with GenericTask("svn update") as t:
    ...
    if svnInfo_local:
      ...
      ret, lines = exec(f'svn update . --force --accept tf --config-option config:miscellany:use-commit-times=yes --non-interactive --username "{vcs.username:}" --password "{vcs.password:}" -r {targetRevision}', callbackClass=pr, optional=True, **execCtx)
      if ret == 0:
