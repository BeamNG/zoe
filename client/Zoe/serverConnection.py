...
class CommThread(threading.Thread):
  ...
  def create_artifact(self, artifact_name, artifact_path):
    msg = {
      'type': 'create_artifact',
      'artifact_name': artifact_name,
    }
    r = requests.post(self.env['SERVER_URL'] + '/artifacts', json=msg)
    if r.status_code != 200:
      raise Exception("Failed to create artifact: {}".format(r.text))
    else:
      logger.info("Artifact created: {}".format(artifact_name))
